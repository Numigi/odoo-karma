...
class ResUsersWithBadgeCount(models.Model):
    ...
    def _register_hook(self):
        ...
        self._register_date_range_field(
            "gamification_goal_ratio", compute_reached_goal_ratio
        )
...
class TestFieldTemplate(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls._assign_badge_at_previous_date(
            cls.env, cls.one_month_ago, cls.badge_1, cls.user
        )
        cls._assign_badge_at_previous_date(
            cls.env, cls.one_week_ago, cls.badge_2, cls.user
        )
        cls._assign_badge_at_previous_date(
            cls.env, cls.today_5_oclock, cls.badge_3, cls.user
        )
        ...
        cls._assign_goal(
            cls.env, cls.one_month_ago, cls.definition_1, cls.user, "reached"
        )
        cls._assign_goal(
            cls.env, cls.one_month_ago, cls.definition_2, cls.user, "reached"
        )
        cls._assign_goal(
            cls.env, cls.one_week_ago, cls.definition_1, cls.user, "reached"
        )
        cls._assign_goal(
            cls.env, cls.one_week_ago, cls.definition_2, cls.user, "failed"
        )
    ...
    def test_goal_ratio_previous_7_days(self):
        self.assertAlmostEqual(
            self.user.x__gamification_goal_ratio__previous_7_days, 0.5, 2
        )

    def test_goal_ratio_previous_30_days(self):
        self.assertAlmostEqual(
            self.user.x__gamification_goal_ratio__previous_30_days, 0.75, 2
        )
    ...
    def test_number_of_badges_today_with_canada_eastern_tz_plus_4_hours(self):
        ...
        with freeze_time(self.today + relativedelta(hours=4, minutes=59, seconds=59)):
            self.assertEqual(
                self.user.with_context(tz=tz).x__gamification_badge_count__today, 0
            )
    ...
    def test_number_of_badges_today_with_canada_eastern_tz_plus_5_hours(self):
        ...
        with freeze_time(self.today + relativedelta(hours=5)):
            self.assertEqual(
                self.user.with_context(tz=tz).x__gamification_badge_count__today, 1
            )
        ...
        field_entry = self.env["computed.field"].create(
            {
                ...
                "range_id": self.env.ref(
                    "date_range_field_template.range_yesterday"
                ).id,
            }
        )
