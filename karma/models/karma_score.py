...
class KarmaScore(models.Model):
    ...
    model_id = fields.Many2one(
        related="karma_id.model_id", auto_join=True, readonly=True
    )
...
class KarmaScoreInheritedDetail(models.Model):
    ...
    child_score_karma_type = fields.Selection(
        related="child_score_id.karma_type", readonly=True
    )
...
class KarmaScoreCondition(models.Model):
    ...
    field_id = fields.Many2one(
        "ir.model.fields", "Field", required=True, index=True, ondelete="cascade"
    )
