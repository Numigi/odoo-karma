...
class Karma(models.Model):
    ...
    def _get_karma_properties_field_name(self):
        return (
            "x_karma_score_" + (self.ref).replace("-", "_").lower(),
    ...
    def add_field_properties(self):
        ...
        self.create_new_field(name=score_fname, ttype="float", model_id=self.model_id)
        self.create_new_field(name=grade_fname, ttype="char", model_id=self.model_id)
    ...
    def write(self, vals):
        ...
        for rec in self:
            if rec.show_properties:
                (score_fname, grade_fname) = rec._get_karma_properties_field_name()
                # Add or Remove tree and search views
                if "show_grade" in vals:
                    rec.add_remove_property(vals["show_grade"], field=grade_fname)
                if "show_score" in vals:
                    rec.add_remove_property(vals["show_score"], field=score_fname)
        return True
